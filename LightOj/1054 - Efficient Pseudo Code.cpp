...
void Prime_Power_Factorization(lli n, lli z)
{
    ...
    for(lli i=0 ; i<prime.size() && prime[i]<=sqrtn ; i++)
    {
        if(n%prime[i]==0)
        {
            ...
            while(n%prime[i]==0)
            {
                n/=prime[i];    p=p*prime[i];   if(z==0)   mp[ prime[i] ]++;
            }
    ...
    if(n!=1)
    {
        sum=sum*(n+1) , mp[n]=1,  factors.push_back(n);
    }
}
...
int main()
{
    ...
    fr1(i,t)
    {
        sf2(x,y);  a=1; factors.clear();  mp.clear();
